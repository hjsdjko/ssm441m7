...
export default {
  data() {
	...
	jiazhengleixing : false,
      ...
      rules: {
          jiazhengleixing: [
                { required: true, message: '家政类型不能为空', trigger: 'blur' },
          ],
      }
    };
  },
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiazhengleixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiazhengleixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
