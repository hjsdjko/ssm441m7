...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussjiazhengrenyuanCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussjiazhengrenyuanCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussjiazhengrenyuan');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussjiazhengrenyuan');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussjiazhengrenyuan', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussjiazhengrenyuan', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussjiazhengrenyuan/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussjiazhengrenyuan', this.detail);
				this.$utils.msg('审核成功');
