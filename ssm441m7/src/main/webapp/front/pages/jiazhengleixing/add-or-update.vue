	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`jiazhengleixing`, this.ruleForm.id);
...
//跨表计算判断
				if((!this.ruleForm.jiazhengleixing)){
					this.$utils.msg(`家政类型不能为空`);
					return
				}
				//更新跨表属性
			       var crossuserid;
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`jiazhengleixing`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`jiazhengleixing`, this.ruleForm);
						}else{
							await this.$api.add(`jiazhengleixing`, this.ruleForm);
						}
