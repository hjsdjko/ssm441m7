...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','jiazhengleixing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','jiazhengleixing');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('jiazhengleixing', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussjiazhengleixing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('jiazhengleixing', this.detail);
				this.$utils.msg('审核成功');
